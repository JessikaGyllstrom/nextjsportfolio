      ...
      <div className="mx-auto flex justify-center w-screen md:mb-24">
        <div className='flex flex-col mt-8'>
          ...
          {aiart.map((aiart, id) =>
            <div key={id} className="flex justify-center text-6xl border-3 border-gray-300 rounded-sm p-2 bg-neutral-800 object-cover">
              <AnimateR>
                <img key={id} className="w-full h-full rounded-sm  hover:scale-105 transition duration-500 cursor-pointer object-cover" src={urlFor(aiart.image.asset._ref).url()} />
              </AnimateR>
            </div> 
            )}
          </div>
